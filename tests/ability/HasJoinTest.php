...
class HasJoinTest extends \b2\tests\Base
{
	public function testToString() {
		...
		$this->assertSame("LEFT JOIN `hi` ON field > fielb2", $j->toString($this->quoter()));

		$j = new SimpleJoinable;
		$j->leftJoin('hi', 'field > ?', [10]);
		$this->assertSame("LEFT JOIN `hi` ON field > '10'", $j->toString($this->quoter()));

		$j->innerJoin('hi2', 'fielb2 > ?', [20]);
		$this->assertSame("LEFT JOIN `hi` ON field > '10' INNER JOIN `hi2` ON fielb2 > '20'", $j->toString($this->quoter()));

		$j = new SimpleJoinable;
		$j->innerJoin(new \b2\literal\PlainSql('some table'), ['a' => 'b', 'c' => 'd']);
		$this->assertSame("INNER JOIN some table ON (`a` = 'b') AND (`c` = 'd')", $j->toString($this->quoter()));
	}
