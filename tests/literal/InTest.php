...
class InTest extends \b2\tests\Base
{
	public function testToString() {
		...
		$this->assertSame("`c` IN('hi')", $in->toString($this->quoter()));

		$in->addCase(new Constant('wo'));

		$this->assertSame("`c` IN('hi', 'wo')", $in->toString($this->quoter()));

		$in = new In(new Identifier('c'), [new Constant('wo')]);
		$this->assertSame("`c` IN('wo')", $in->toString($this->quoter()));

		$in = new In(new Identifier('c'), [new Constant('hi'), new Constant('wo')]);
		$this->assertSame("`c` IN('hi', 'wo')", $in->toString($this->quoter()));
		$in->addCase(new Identifier('col'));
		$this->assertSame("`c` IN('hi', 'wo', `col`)", $in->toString($this->quoter()));
	}
